...
@testset "BayesianNonparametricStatistics package" begin
    @testset "model.jl" begin
        ...
        @test M.σ == identity
        σ2(x) = 1.0
        M = SDEModel(σ2, 0.0,10.0,0.01)
        @test M.σ == σ2

        σ3(x) = 2+sin(x)
        M = SDEModel(σ3, 1.3, 101.0, 0.5)
        @test M.σ == σ3
    ...
    @testset "basisfunctions.jl" begin
        ...
        # The function fourier(0) is constant 1.0
        for x in -10.0:0.01:10.0
            @test fourier(0)(x) == 1.0
        end 
        #
        # For k≥1, the functions have minimum -sqrt(2) and maximum sqrt(2).
    ...
    @testset "gaussianvectors.jl" begin
        ...
        @test 2.0 * randomvector[1] ≈ randomvector[2]
        m = [1.0, 2.0]
        Σ = [1.0 0.0; 0.0 2.0]
        n = 100000
        GV = GaussianVector(m, Σ)
        nrandomdraws = Array{Float64, 2}(n, 2)
        for k in 1:n
            nrandomdraws[k, :] = rand(GV)
        end
        mempirical = mean(nrandomdraws, 1)
        vempirical = var(nrandomdraws, 1) 
    ...
    @testset "gaussianprocess.jl" begin
        ...
        @test_throws AssertionError FaberSchauderExpansionWithGaussianCoefficients(Vector{Float64}(0))
