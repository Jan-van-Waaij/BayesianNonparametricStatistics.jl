...
function calculateboundssupport(higestlevel::Int)
    ...
    leftboundsupport = Vector{Float64}(lengthvector)
    rightboundsupport = Vector{Float64}(lengthvector)
    ...
    for j in 1:higestlevel
        ...
        for k in 1:twotothepowerj
            i = twotothepowerj+k
            leftboundsupport[i] = (k-1)*twotothepowerminusj
            rightboundsupport[i] = k*twotothepowerminusj
        end
    end
...
function FaberSchauderExpansionWithGaussianCoefficients(
    ...
    vectorofstandarddeviations = repmat(standarddeviationsperlevel[1:1],2)
    # and 2^k of level k, k=1,2,...
    for k in 1:higestlevel
        vectorofstandarddeviations = vcat(vectorofstandarddeviations, repmat(
            standarddeviationsperlevel[k+1:k+1],2^k))
    end
    distribution = GaussianVector(sparse(Diagonal(vectorofstandarddeviations)))
