...
isincreasing(x::S) where S<:Range{T} where T <: Number = step(x) > 0
...
"""
 ...
 Performance hint: Use Range objects instead of vectors as much as possible, this
...
struct SamplePath{S, T} <: AbstractSamplePath where {S<:AbstractVector{Float64},
  ...
  function SamplePath(timeinterval::S, samplevalues::T) where {
    ...
    length(timeinterval) == length(samplevalues) ||
      throw(DimensionMismatch("Length of timeinterval should be equal
       to the length of the samplevalues vector"))
...
step(X::SamplePath{S}) where S<:Range{T} where T<:Number = step(X.timeinterval)
