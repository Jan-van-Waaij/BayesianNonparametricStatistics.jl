...
Base.iterate(rX::Base.Iterators.Reverse{SamplePath{S,T}}, state=length(X)) where {S<:AbstractVector{Float64},
        T<:AbstractVector{Float64}} = state < 1 ? nothing : (X.samplevalues[state], state - 1) 
